...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var categories = [];
	var query = [   {$group: {_id: "$category", num: {$sum: 1}}}, 
			{$sort: {_id: 1}}
		    ];
	...
	this.db.collection('item').aggregate(query).toArray(function(err, docs){
		...
		categories.push({_id: "All", num: all });
 		callback(categories);
	});
    }


    this.getItems = function(category, page,  PerPage, callback) {
        "use strict";
	...
	this.db.collection('item').find(query).skip(page*PerPage).limit(PerPage).toArray(function(err, docs){
		callback(docs);
	});
    }
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        *
        */
      this.db.collection('item').find({$text: {$search: query}}).count(function(err, docs){
        callback(docs);
      });
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
      ...
      this.db.collection('item').updateOne({_id: itemId}, {$push: {"review": reviewDoc}}, function(err, docs) {
          callback(docs);
      });
    }
